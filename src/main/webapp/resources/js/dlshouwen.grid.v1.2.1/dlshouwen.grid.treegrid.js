		...
		init : function(options, name) {
			...
			var gridObject = {
				...
				load : function(){
					...
					if(gridReflectionObj.init.toolsIsInit==false){
						...
						if(!gridReflectionObj.option.ajaxLoad){
							gridReflectionObj.hideProcessBar(function(){
								...
								gridReflectionObj.originalDatas = gridReflectionObj.originalDatas?gridReflectionObj.originalDatas:[];
								//处理基础数据集
							...
							if(gridReflectionObj.option.loadAll){
								...
								{
								    param = gridReflectionObj.parameters?gridReflectionObj.parameters:{};
								}
								$.ajax({
									type:'post',
									url:url,
									...
									success:function(datas){
										...
										gridReflectionObj.hideProcessBar(function(){
											//处理原始数据集
											gridReflectionObj.originalDatas = datas;
											gridReflectionObj.originalDatas = gridReflectionObj.originalDatas?gridReflectionObj.originalDatas:[];
											//处理基础数据集
											gridReflectionObj.baseDatas = gridReflectionObj.originalDatas.slice(0, gridReflectionObj.originalDatas.length);
											//处理分页属性
					...
					if(!gridReflectionObj.option.ajaxLoad){
					    ...
					    if(gridReflectionObj.option.loadAll)
					    {
                            ...
                            pager.parameters = gridReflectionObj.parameters?gridReflectionObj.parameters:{};
                            pager.fastQueryParameters = gridReflectionObj.fastQueryParameters?gridReflectionObj.fastQueryParameters:{};
                            pager.advanceQueryConditions = (gridReflectionObj.advanceQueryParameter&&gridReflectionObj.advanceQueryParameter.advanceQueryConditions)?gridReflectionObj.advanceQueryParameter.advanceQueryConditions:[];
                            pager.advanceQuerySorts = (gridReflectionObj.advanceQueryParameter&&gridReflectionObj.advanceQueryParameter.advanceQuerySorts)?gridReflectionObj.advanceQueryParameter.advanceQuerySorts:[];
                            var params = {};
                            ...
                            $.ajax({
                                ...
                                success:function(pager){
				            		...
				            		{
				            			$(".page-content").empty();//清除该节点子元素
				            			layer.msg(pager.message, {icon : 0});
				            			//window.location.href = sys.rootPath + pager.url;
				            			return;
				            		}else if(pager.status == "403")
				            		{
				            			layer.confirm(pager.message, {
						                    icon : 3,
						                    title : '提示',
		                    				btn: ['重新登录','取消'] //按钮
						                }, function(index, layero) {
				...
				constructTreeGrid : function(datas, dataNo, level){
                    ...
                    tree_originalDatas = tree_originalDatas ? tree_originalDatas : [];
                    //处理基础数据集
					...
					$('#dlshouwen_grid_'+gridId+' .can-sort').click(function(){
						...
						if(gridReflectionObj.sortParameter==null){
							gridReflectionObj.sortParameter = {};
						}
				...
				constructGrid : function(){
					...
					if(gridReflectionObj.exhibitDatas!=null){
						...
						for(var i=0; i<gridReflectionObj.exhibitDatas.length; i++){
							gridContent += '	<tr class="dlshouwen-grid-row" level="0" dataNo="'+i+'">';
							if(gridReflectionObj.option.extraWidth!=null){
							...
							for(var j=0; j<columns.length; j++){
								if(columns[j].width!=null)
								{
									if(columns[j].id == gridReflectionObj.option.iconColumn)
									{
										gridContent += '	<td indexKey="'+gridReflectionObj.exhibitDatas[i][gridReflectionObj.option.indexKey]+'" width="'+columns[j].width+'" dataNo="'+i+'" columnNo="'+j+'" class="dlshouwen-grid-cell '+gridReflectionObj.getColumnClassForHide(columns[j])+' '+columns[j].columnClass+'" style="'+columns[j].columnStyle+'">';
									}else
									{
									...
									if(columns[j].id == gridReflectionObj.option.iconColumn)
									{
										gridContent += '	<td indexKey="'+gridReflectionObj.exhibitDatas[i][gridReflectionObj.option.indexKey]+'" dataNo="'+i+'" columnNo="'+j+'" class="dlshouwen-grid-cell '+gridReflectionObj.getColumnClassForHide(columns[j])+' '+columns[j].columnClass+'" style="'+columns[j].columnStyle+'">';
									}else
									{
								...
								if(columns[j].resolution){
									value = columns[j].resolution(value, gridReflectionObj.exhibitDatas[i], columns[j], gridReflectionObj, i, j);
								}else{
									value = gridReflectionObj.formatContent(columns[j], value);
								}
								if(gridReflectionObj.option.isTreeGrid)
								{
									if(columns[j].id == gridReflectionObj.option.iconColumn)
									{
										if(!gridReflectionObj.exhibitDatas[i][gridReflectionObj.option.isLeafColumn])
										{
											value = $.fn.dlshouwen.grid.lang[gridReflectionObj.option.lang].extraColumn.carent_right + value;
										}
										value = '<div style="overflow:hidden;">' + value + '</div>';
									}
								}
								gridContent += value;
								gridContent += '	</td>';
							...
							gridContent += '	</tr>';
							gridContent += '	<tr level="0" id="dlshouwen_grid_'+gridReflectionObj.option.id+'_extra_columns_'+i+'" class="dlshouwen-grid-extra-columns hidden">';
				...
				doFastQuery : function(form){
					...
					var gridReflectionObj = this;
					//清空快速查询中的参数
					gridReflectionObj.fastQueryParameters = {};
					//遍历获取快速查询参数
...
//					});
				...
				getCheckedRecords: function(){
					...
					$('input[id*=dlshouwen_grid_'+gridReflectionObj.option.id+'_check_]:checked').each(function(){
						//1、treegrid需要单独获取；
						if($(this).attr('indexKey'))
						{
							records.push(gridReflectionObj.treegrid_exhibitDatas[$(this).attr('indexKey')]);
						}else{
